...
public class AIPlayer implements Player {
     ...
     *
     * La IA simula tiempos de espera para voltear
     * cartas y luego evalúa si hay coincidencia.
     */
    @Override
   ...
   private List<Position> calculateBestMove(Board board) {
    ...
    /**
     * Ejecuta una jugada de forma síncrona.
     *
     * Se usa únicamente para pruebas.
     */
     ...
     * @param board tablero actual
     * @param first primera carta seleccionada
     * @return posición de la segunda carta
     */
    ...
    private void updateMemory(Board board, Position first, Position second) {
        for (Position pos : List.of(first, second)) {
            Card card = board.getCard(pos);
            if (!card.isMatched()) {
                knownCards.put(card.getId(), pos);
            } else {
                knownCards.remove(card.getId());
            }
        }
    }
}
